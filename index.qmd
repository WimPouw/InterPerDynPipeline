...
subtitle: "Theory-driven Measurement, Analysis, and Archiving"
...
#### Notes on tested system specifications
...
!["Figure S1. Overview demographics Sibling dataset"](./images/masked_example.jpeg){fig-align="center" width=600}
:::

### Case 1: Dataset siblings
...
!["Figure S2. Overview demographics caregiver-infant dataset"](images/mother_infant_analysis.png){fig-align="center" width=600}
...
# Step 1: Motion tracking, signal processing, and wrangling
...
def check_for_duplication(results):
    ...
    close_threshold =150 # note that the value is in pixels
    # missing data tolerance
    miss_tolerance = 0.95 # this means we can miss up to 95% of the keypoints
    drop_indices = []
    if len(results[0].keypoints.xy) > 1: # only proceed if a skeleton is detected
        conf_scores = []
        ...
        for person in range(len(results[0].keypoints.xy)):
           ...
           perc_missed = keypoints_missed/len(tensor_to_matrix(results[0].keypoints.xy[person])) # calculate percentage of missing keypoints
           
           # compare % of missing keypoints against the threshold that we set earlier
...
```{python,eval=TRUE, code_folding="hide"}
import pandas as pd
import numpy as np
import glob as glob
import os
...
# get raw speed values
raw_speed = df['wrist_left_p1_speed'].values
# smooth the speed timeseries
...
![Figure S4. Effect of smoothing on speed data using Savitsky-Golay filter](./images/smoothing_speed_effect.png)
...
![Figure S5. Proximity Calculation Visualization](./images/proximity_visualization/proximity_calculation.gif)
...
![Figure S6. Animation of motion tracking data coupled with video](./images/com_distance_example.gif)
...
![Figure S7. Smoothness metrics comparison](./images/smoothness_metrics_comparison.png)
...
{{< include ./code_STEP3_nonlinear_analysis/STEP3_nonlinearfeatures_CRQA.html >}}
:::
...
Before running the markdown file you will need to make sure you have a few packages installed (run this in R or R Studio):
*install.packages(c("rio","plyr","tidyverse","invctr","fpCompare","gplots","testthat"))*
...
## Step 4.1: Statistical report on smoothness analysis in R

For the individual level analysis of the smoothness of the individual approach/avoidance behavior, we fit a linear mixed model using `lme4` [@batesFittingLinearMixedEffects2015] where the individual smoothness metric was the outcome variable, culture (Yurakaré or Polish) was the predictor variable, and we included a random intercept to account for individuals being nested within sibling pairs. Overall, the model suggests there was no evidence of an effect of culture on the smoothness of individual approach/avoidance movements, (β = 0.12, 95% CI [−0.53, 0.77], marginal R² = .004). For the Polish group, the estimated marginal mean was 38.3 (SE = 0.45, 95% CI [37.3, 39.2]). For the Yurakaré group, the estimated marginal mean was 39.5 (SE = 0.45, 95% CI [37.6, 49.5]). The figure below shows the overall pattern. In order to evaluate whether the results of these models were biased due to singular fit issues in estimating the random effects, we ran an equivalent linear regression including cluster robust standard error estimates [@mcneishUnnecessaryUbiquityHierarchical2017b] using the `clubSandwich` [@pustejovskyClubSandwichClusterRobustSandwich2025] `R`-package, which provided compatible results. 
...
{{< include ./code_STEP4_statisticalanalysis/STEP4_statisticalanalysis_CRQA.html >}}
...
<html>
<video width="600" height="400" controls>
  <source src="./images/contoursvideosample.mp4" type="video/mp4">
  Your browser does not support the video tag.
</video>
</html>
...
|----------|-------------|---------------|---------------------|
| **Blur** | Applies Gaussian blurring at a user-controllable blur intensity level. | Moderate, may be limited depending on blur intensity. | Progressively blurred versions maintaining color and general structure ![Blur example](./images/blur_apa.png)  |
| **Pixellation** | Reduces image resolution by grouping pixels into uniform blocks, creating a mosaic effect | Moderate to high, depending on block size | Blocky, mosaic-like appearance with preserved general shapes  ![Pixel example](./images/pixellation_apa.png) |
| **Contours** | Uses Canny edge detection to preserve essential contours while eliminating finer details. User-controlled detail. | Moderate, balances privacy with utility. | Black and white line drawings showing only structural boundaries ![Contour example](./images/countours_apa.png) |
...
## Ethical Considerations: Need for Integration in Research Ethics Protocols and Informed Consent
...
In earlier work of the masking project we utilized a light-weight masking approach using Mediapipe [@lugaresiMediaPipeFrameworkBuilding2019] called MaskedPiper [@owoyeleMaskedPiperMaskingPersonal2022]. We have recently provided an updated and enriched python notebook that allows for masking single-person videos in several ways. 

You can find the code on www.envisionbox.org in one of the modules called ["Full-body tracking, +masking/blurring, and movement tracing (Python)"](https://envisionbox.org/embedded_Mediapiping.html).

![MaskedPiper 2.0 example](./images/ted_kid_blurred.gif)
:::

::: {.callout-note .callout-settingup collapse="true"}
## DIY: Trying masking yourself (Advanced) 🛠️
MaskAnyone is the most recent release version of the masking project [@owoyeleMaskAnyoneToolkitOffering2024]. The MaskAnyone code is available on GitHub: [MaskAnyone](https://github.com/MaskAnyone/MaskAnyone) and there is an installation guide available on the GitHub page.

![MaskAnyone 2.0 example](./images/maskanyone.gif)
:::

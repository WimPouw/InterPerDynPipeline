...
The purpose of this Quarto notebook is to provide a detailed overview of the analysis processing and analysis steps. Rather than being purely written description, the notebook is intended to combined written description with several data explanation formats:

- **code snippets** that highlight key portions of code, for example to explain a particiular function, highlight a potentially highly-adaptable plotting function, etc.

- **example output** that shows what type of output you can expect, and to directly link these outputs with labels and descriptions of (e.g., how to interpret a plot, what variables in a data table represent)

- **DIY blocks** that provide shorter, step-by-step links to the full code for executing a particular step in the pipeline on your own code
...
To increase privacy we apply maskanyone [@owoyeleMaskAnyoneToolkitOffering2024].<br>
...
```bash
cd [yourspecificrootadress]/code_STEP1_posetrackingprocessing/
python STEP1_1_yolo_tracking_processing.py
```
:::
...
::: {.callout-note .callout-settingup collapse="true"}
## DIY: Post-Processing 🛠️
Assuming you installed the requirements back in the previous step (DIY: Motion Tracking Set-Up and Execution), you just need to run the python script for Step1_2. Otherwise, make sure to also run *pip install -r requirements.txt*

```bash
py ./code_STEP1_posetrackingprocessing/STEP1_2_yolo_tracking_social_proximity.py
...
After generating a time-series that represents interaction dynamic that we are interested in (i.e., approach, as calcualted in the `p1_com_approach_pos` and `p2_com_approach_pos` variables), we can calculate higher-level features by transforming or aggregating the data. In the examples given in this notebook, we demonstrate two approaches: one that uses the data that we calculated in Step 2, without further transformation, to perform time-series focused analyses of non-linear dynamics. The second approach involves first transforming and then aggregating the time-series once more.

**From raw proximity to *smoothness of proximity***:
While the time-series that we got in the previous step provides the proximity of two individual's center of mass, we actually want something that is 1) capturing the smoothness of their *coordination*, and 2) generalizable between slightly different recording conditions. This second point is particularly important when comparing interactions captured in an unconstrained way, and/or when captured in different studios/locations . 

First, we will take the proximity time-series and calculate jerk. Jerk is the third derivative of our proximity time-series. We therefore take the *change* in proximity from timepoint to timepoint, divided by the time interval. Doing this once, on the proximity values, gets us the first derivative: speed. The derivative of speed is calculated the same way, giving us *acceleration*, or how fast the speed is changing from timepoint to timepoint. The derivative of acceleration is then *jerk*. We utilize jerk here because of its theoretical importance in human movement sciences, for example in modeling complex biomechanical movements as well as interpersonal coordination. 

While jerk-minimization has been a popular measure, it is affected by the amplitude and duration of movements, which can make comparisons between different movements challenging. A solution is to calculate dimensionless (squared) jerk. The calculation results in a single (individual-level), positive value. The mathematical underpinnings of this calculation are far outside the scope of these examples, and the accompanying manuscript. However, what is important to note here is that the measure (dimensionless jerk) was based on theoretical reasoning relating to our primary research questions. 

The plot below shows the relationship between the three initial measures: speed, acceleration, and jerk.
```{python, eval=False, code_folding="hide"}
import pandas as pd
import os
import numpy as np
...
os.path.isdir('./dataoutput_STEP1_1_rawposedata')
ts = pd.read_csv('./dataoutput_STEP1_1_rawposedata/sample_keypoints_data_layer1.csv')
...
raw_speed, speed, raw_acceleration, acceleration, raw_jerk, jerk = dimensionless_squared_jerk_from_position(ts['p1_com_approach_pos'].values, ts['time'].values)
plt.plot(normalize_array(speed[0:150]))
plt.plot(normalize_array(acceleration[0:150]))
plt.plot(normalize_array(jerk[0:150]))
plt.legend(labels=["speed","acceleration","jerk"])
plt.show()
```

An additional methodological note, is the need for smoothing the data. Motion tracking, even when quite accurate, is prone to small fluctuations in position from frame to frame even when the tracked object/person is not moving. When we calculate something like interpersonal proximity, these fluctuations will cause fluctuations in proximity. This transfers to a subsequent measures, such as speed, acceleration, and jerk. In the provided pipeline, we use a Savitsky-Golay filter (implemented by SciPy). The filter slides along the time-series, smoothing the data within a particular window-length by fitting a polynomial of a pre-determined order to the datapoints. In our case, we set a time window of 11 datapoints, and a third degree polynomial. The plot below shows the effect of smoothing the speed data in this way.

```{python, eval=False, code_folding="hide"}
plt.plot(normalize_array(raw_speed[0:150]))
plt.plot(normalize_array(speed[0:150]))
plt.legend(labels=["raw speed","smoothed speed"])
plt.show()
```
::: {.callout-note .callout-settingup collapse="true"}
...
### Step 2: run the feature extraction script
Once everything is installed, you can run the code that extracts the smoothness features based on dimensionless jerk:
```bash
py ./code_STEP2_features/STEP2_featureextraction.py
```

:::

# Step 4: Statistical analysis (e.g., jerkiness of proximity/approach/avoidance, or non-linear recurrence) (R)
For the first analysis, we are simply running a linear mixed model to assess whether there is an effect of *culture* on *smoothness*. The code below adds a grouping variable, based on substrings in the videoID variable, then convers the dataframe from wide to long format in order to accommodate the lme4 syntax. We include random intercepts for videoID in order to capture some of the variance between different dyads within the same culture.

```{r child = "../code_STEP4_statisticalanalysis/STEP4_Smoothness_analysis.Rmd.Rmd"}
```

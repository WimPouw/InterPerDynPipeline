...
For this step, we simply need to run the corresponding python scripts:
```bash
py ./code_STEP3_features/STEP3_featureextraction.py
...
After generating a time-series that represents interaction dynamic that we are interested in (i.e., interpersonal proximity, in the case of this example), we can calculate higher-level features by transforming or aggregating the data. In the examples given in this notebook, we demonstrate two such approaches. The first approach results in data that is aggregated at the dyad level (I.e., resulting in one value per dyad), while the other approach results in a new time-series, equal in length to the original, but representing a different dynamical feature.
...
While jerk-minimization has been a popular measure, it is affected by the amplitude and duration of movements, which can make comparisons between different movements challenging. A solution is to calculate dimensionless (squared) jerk. The mathematical underpinnings of this calculation are far outside the scope of these examples, and the accompanying manuscript. However, what is important to note here is that the measure (dimensionless jerk) was based on theoretical reasoning relating to our primary research questions. The measure is also quite useful in that it can be taken as a time-series (as will be used in the CRQA portion of step 4), or it can be aggregated to inform dyad-level analyses, as will be done in the linear analysis portion of step 4.

**Dyad-Level Aggregation**
For now, let's move on to aggregation....

```{python, eval=FALSE, code_folding="hide"}
time_series = pd.read_csv('./data_timeseries_afterSTEP2/sample_processed_data_layer2.csv')
